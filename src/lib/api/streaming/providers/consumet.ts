...
export async function fetchEpisodes(animeId: string): Promise<EpisodesResponse> {
  try {
    logger.info(`Fetching episodes from Consumet for anime: ${animeId}`);
    ...
    logger.error('Consumet episodes fetch failed:', error);
    throw error;
  }
}
...
export async function fetchStreamingSources(episodeId: string): Promise<StreamingData> {
  try {
    logger.info(`Fetching streaming sources from Consumet for episode: ${episodeId}`);
    ...
    logger.error('Consumet streaming sources fetch failed:', error);
    throw error;
  }
}